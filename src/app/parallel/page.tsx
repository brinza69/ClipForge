...
import {
  Layers, Loader2, Download, AlertCircle, CheckCircle2, Eraser, FileText, Plus, HardDrive,
} from "lucide-react";
...
export default function ParallelPage() {
  ...
  const [drive, setDrive] = useState<{ connected: boolean; client_configured: boolean; email: string | null }>({ connected: false, client_configured: false, email: null });
  ...
  const connectDrive = async () => {
    ...
    try {
      ...
      // Poll until the background loopback server saves the token.
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-5 p-6">
      ...
      {/* Google Drive connection — uploads use YOUR account quota */}
      <Card className="p-4 border-border/40">
        <div className="flex items-center gap-3">
          ...
          {drive.connected ? (
            <Button size="sm" variant="outline" onClick={disconnectDrive}>Disconnect</Button>
          ) : (
            <Button size="sm" onClick={connectDrive} disabled={driveConnecting || !drive.client_configured}>
              {driveConnecting ? <Loader2 className="h-4 w-4 animate-spin" /> : "Connect Google Drive"}
            </Button>
          )}
        </div>
      </Card>

      {/* 2. Shared zone picker — identical to Remix */}
      {preview && (
        <ZonePicker
          preview={preview}
      ...
      {preview && (
        <Card className="p-4 space-y-3 border-border/40">
          <div className="flex items-center justify-between">
            ...
            {!isRunning && (
              <Button onClick={start}><Layers className="h-4 w-4 mr-1" /> Process {variants.length} videos</Button>
            )}
          </div>
