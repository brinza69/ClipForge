...
if (-not $py312Found) {
    ...
    $issues += "Python 3.12: NOT FOUND"
    $issues += "  -> faster-whisper requires Python 3.10-3.12 (3.14 is NOT compatible)"
    $issues += "  -> Install from: https://www.python.org/downloads/release/python-3129/"
    $issues += "  -> During install check 'Add to PATH' and 'py launcher'"
    if ($pyList.Trim()) {
        $issues += "  -> Available versions: $($pyList.Trim())"
    }
}
...
try {
    ...
    if ($gpuInfo) {
        ...
        $warnings += "GPU: No NVIDIA GPU detected - transcription will use CPU (slower)"
    }
} catch {
    $warnings += "GPU: nvidia-smi not found - transcription will use CPU (slower)"
}
...
try {
    ...
    if ($drive) {
        ...
        if ($freeGB -lt 10) {
            $warnings += "Disk Space (D:): ${freeGB}GB free - recommend at least 20GB"
        } else {
