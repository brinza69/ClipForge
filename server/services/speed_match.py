...
def match_video_to_voice(
    ...
    *,
    ...
    """
    Speed/slow the video so its duration equals the voice's, then drop the
    source video's audio and mux the TTS voice in unchanged. Returns a stats
    dict with the durations and the factor we applied.

    NOTE: the remix/parallel pipelines now fuse this with the caption burn
    (one encode instead of two). This standalone encode remains for any caller
    that wants a speed-matched file on its own.
    """
    plan = compute_speed_plan(video_path, voice_path)
    factor = plan["factor"]
    voice_dur = plan["voice_dur"]
    video_dur = plan["video_dur"]
    stretched_dur = plan["stretched_dur"]
    overhang = plan["overhang_s"]
    clamped = plan["clamped"]
    vfilter = plan["vfilter"]
    ffmpeg = _ffmpeg_bin()
